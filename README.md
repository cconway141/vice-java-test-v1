...
GET /jobs/2/schedule
200 {"jobId":2,"customerId":"cust-100","status":"on_hold","scheduledStart":null}
```
...
Three users, all with password `password`:

| username | realm role | customer_id claim |
|---|---|---|
| builder1 | builder | none |
| customer100 | customer | cust-100 |
| customer200 | customer | cust-200 |
...
Postgres is at `localhost:5432`, database `homebuilder`, user `homebuilder`, password `homebuilder`. Keycloak's issuer URI is `http://localhost:8081/realms/homebuilder`.
